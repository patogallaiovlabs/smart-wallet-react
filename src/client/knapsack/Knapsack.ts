/**
 * Copied from https://raw.githubusercontent.com/sb-js/typescript-algorithms/master/src/algorithms/sets/knapsack-problem/Knapsack.js
 */

import MergeSort from 'src/client/knapsack/sort/MergeSort';


class Item {
  weight:number = 0;
  value:any;
  valuePerWeightRatio:any;
}
...
export default class Knapsack {
   ...
   */
  constructor(possibleItems:Item[], weightLimit:number) {
    this.selectedItems = [];
  ...
  sortPossibleItemsByWeight() {
    this.possibleItems = new MergeSort({
       ...
       */
      compareCallback: (itemA:Item, itemB:Item) => {
  ...
  solveZeroOneKnapsackProblem() {
    console.log('solving knapsack problem...');
    ...
    for (let weightIndex = 1; weightIndex <= this.weightLimit; weightIndex += 1) {
      const itemIndex = 0;
      if (this.possibleItems.length > 0 && this.possibleItems[itemIndex]) {
