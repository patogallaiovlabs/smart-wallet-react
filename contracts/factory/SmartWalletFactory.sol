...
contract SmartWalletFactory is ISmartWalletFactory {
    ...
    function createUserSmartWallet(
        ...
        bytes memory _hash = keccak256(abi.encodePacked(
            owner,
            recoverer,
            index
        ));
        bytes memory message = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", _hash));
    ...
    function deploy(
        ...
        emit AboutToDeploy(addr, uint256(salt));
